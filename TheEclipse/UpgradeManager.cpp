...
void UpgradeManager::Initialize(UpgradeButton* btn1, UpgradeButton* btn2, ExpGauge* gauge, Player* player)
{
    ...
    AddStatusUpgrade(L"강화마법", L"공격력을 1 증가", L"", UpgradeStatusType::Attack, 1);
    AddStatusUpgrade(L"신속", L"이동 속도를 1 증가시킵니다", L"", UpgradeStatusType::MoveSpeed, 1);
    AddStatusUpgrade(L"다중 탄환", L"마법 탄환 1개 증가", L"", UpgradeStatusType::BulletMultiple, 1);
    AddStatusUpgrade(L"빠른 시전", L"마법 영창 시간 단축", L"", UpgradeStatusType::FireSpeed, 4);
...
{
    upgradeList.push_back(new StatusUpgrade(name, description, textureName, _player, value, type));
}
