...
void BaseWindow::createWindow()
{
	...
	m_hWnd = CreateWindowW(
		...
		L"TheEclipse",   // 제목
		WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, // 윈도우 어떤 스타일로 만들것인가
		Winposx,  // ★ 띄울 위치의 LEFT
		Winposy,             // ★ 띄울 위치의 TOP
		SCREEN_WIDTH,             // ★ 해상도X
		SCREEN_HEIGHT,             // ★ 해상도Y
	...
	HICON hIconBig = LoadIcon(m_hInst, MAKEINTRESOURCE(IDI_ICON2));
	SendMessage(m_hWnd, WM_SETICON, ICON_BIG, (LPARAM)hIconBig);

	// 작은 아이콘 설정 (창 제목 표시줄 아이콘)
	HICON hIconSmall = LoadIcon(m_hInst, MAKEINTRESOURCE(IDI_ICON2));
